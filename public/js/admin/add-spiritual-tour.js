...
$(document).ready(function ()
{
    $("body").on("change","#states",function(){
        ...
        postData('/admin/tours/get-cities',{"state_id":stateId,'tour_type':4},function(response){
        ...
        var cityId=$(this).val();
        $(".city-wrapper").removeClass("hidden");
        postData('/admin/tours/tourism-places-add-price',{"city_id":cityId,'tour_type':4},function(response){
            var options='';
            if(response.success)
            {
    ...
    }).on("click","#addSpiritualTour",function(){
        ...
        if(stateId=='')
        {
           ...
           if(placeName=="")
           {
               messageDisplay("Plase Select Places");
               return false;
           }
        ...
        ajaxData('/a_dMin/add-spiritual-tour',formData,function(response){
            if(response.success)
            {
                messageDisplay(response.message);
                setTimeout(function(){
                    window.location.href=BASE_URL+"/a_dMin/spiritual-tour-list";
