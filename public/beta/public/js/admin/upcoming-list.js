...
$(document).ready(function () {
        ...
        if(items>10) {
            $('.pagination').pagination({
                ...
                onPageClick: function (pageNumber) {
                    ...
                    ajaxCall = $.ajax({
                        type: "POST",
                        url: BASE_URL + '/admin/admin/load-upcoming-list',
                        data: formData,
                        cache: false,
                        contentType: false,
                        processData: false,
    ...
    initPagination();
    $("body").on("click",".delete-upcoming",function () {
        var upcomingId=$(this).data("id");
        $(".delete-conform-button").attr("data-id",upcomingId);
        $("#deletePlaceModal").modal("show");
    }) .on("click",'.delete-conform-button',function () {
        $(this).prop("disabled",true);
        var upId = $(this).attr("data-id");
        postData("/admin/admin/delete-upcoming", {upcoming_id:upId},function (response){
