...
$(document).ready(function () {
        ...
        if(items>10)
        {
            $('.pagination').pagination({
                ...
                onPageClick: function (pageNumber) {
                    ...
                    ajaxCall = $.ajax({
                        type: "POST",
                        url: BASE_URL + '/admin/admin/load-inbox-list',
                        data: formData,
                        cache: false,
                        contentType: false,
                        processData: false,
    ...
    $("body").on("click",".delete-place",function () {
        ...
        $("#deletePlaceModal").modal("show");
          ...
          id=$(this).data("id");
            console.log(id);
         postData("/a_dMin/inbox-single",{"inbox_id":id},function(response){
                        $("#inboxMessage").html(response);
                        $("#inboxpopupModal").modal('show');
         });
    }).on("click","#sendReply",function(){
        var message=$("#message-text").val();
        var element=$(this);
        element.html('Please wait...');

        element.prop('disabled',true);
        postData("/a_dMin/send-message",{"inbox_id":id,'message':message },function(response){
        ...
        postData("/admin/admin/delete-country", {country_id:countryId},function (response){
            var jsonRespnse = parseJsonData(response);
            messageDisplay(jsonRespnse.message);
            if(jsonRespnse.success) {
                setTimeout(function(){
                    window.location.href=BASE_URL+"/a_dMin/inbox";
                },3000);
            }else {
