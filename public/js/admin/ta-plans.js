...
$(document).ready(function () {
        ...
        if(items>10) {
            $('.pagination').pagination({
                ...
                onPageClick: function (pageNumber) {
                    ...
                    ajaxCall = $.ajax({
                        type: "POST",
                        url: BASE_URL + '/admin/admin/load-all-ta-plans',
                        data: formData,
                        cache: false,
                        contentType: false,
                        processData: false,
    ...
    $("body").on("click",".delete-plan",function () {
        ...
        $(".delete-conform-button").attr("data-id",planId);

        $("#deletePlanModal").modal("show");
    }) .on("click",'.delete-conform-button',function () {
        $(this).prop("disabled",true);
        var planId = $(this).attr("data-id");
        postData("/admin/admin/delete-ta-plan", {plan_id:planId},function (response){
