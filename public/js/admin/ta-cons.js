...
$(document).ready(function () {
    ...
    $('table').on('click', 'tbody tr', function(){
        ...
        if(id)
        {
            window.location = BASE_URL+'/a_dMin/se-tas/'+id;
        }
    });
        ...
        if(items>10) {
            $('.pagination').pagination({
                ...
                onPageClick: function (pageNumber) {
                    ...
                    ajaxCall = $.ajax({
                        type: "POST",
                        url: BASE_URL + '/admin/admin/load-ta-cons',
                        data: formData,
                        cache: false,
                        contentType: false,
                        processData: false,
    ...
    initPagination();
    $("body").on("click",".delete-tac",function () {
        var tacId=$(this).data("id");
        $(".delete-conform-button").attr("data-id",tacId);

        $("#deleteTACModal").modal("show");
    }) .on("click",'.delete-conform-button',function () {
        $(this).prop("disabled",true);
        var tacId = $(this).attr("data-id");
        postData("/admin/admin/delete-ta-cons", {tac_id:tacId},function (response){
