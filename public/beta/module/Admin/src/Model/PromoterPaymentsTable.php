    ...
    public function getPromotersPaymentsDetails($data=array('limit'=>10,'offset'=>0)){ 
        try
        {
            ...
            $query = $sql->select()
                ...
                ->join(array('u'=>'users'), 'u.user_id=pp.sponsor_id',array("user_name","mobile","mobile_country_code"),'left')
                ->join(array('pr'=>'promoter_parameters'), 'pr.id<=pp.promoter_id', array("pay_after_pwds", "amt_per_pwd", "redeem_value", "zero"))
                ->join(array('pd'=>'promoter_details'), 'pd.user_id=pp.promoter_id and (r.trigger_payment >= pr.pay_after_pwds or pp.due_amount > pr.zero)', array("bank_ac_no","ifsc_code", "redeem"))
