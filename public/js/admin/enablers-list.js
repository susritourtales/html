        ...
        if(items>10) {
            $('.pagination').pagination({
				...
				onPageClick: function(pageNumber) {
					...
					ajaxCall = $.ajax({
						type: "POST",
						url: BASE_URL + '/a_dMin/load-enablers-list',
						data: formData,
						cache: false,
						contentType: false,
						processData: false,
    ...
    $("body").on('change', '.sd',  function (){
        ...
        postData("/a_dMin/edit-enabler",{'id':id, 'csd':csd},function(response){
            if(response)
            {
                ...
                setTimeout(function(){
                    window.location.href=BASE_URL+"/a_dMin/enablers-list";
                },2000);
            }
        });
...
document.addEventListener('DOMContentLoaded', function () {
    var rows = document.querySelectorAll('tr.ppd');
    rows.forEach(function (row) {
        row.addEventListener('click', function (e) {
            var target = e.target;
            if (!target.closest('td').classList.contains('no-click')) {
