...
$(document).ready(function () {
        ...
        if(items>10) {
            $('.pagination').pagination({
                ...
                onPageClick: function (pageNumber) {
                    ...
                    ajaxCall = $.ajax({
                        type: "POST",
                        url: BASE_URL + '/admin/admin/load-ta-list',
                        data: formData,
                        cache: false,
                        contentType: false,
                        processData: false,
    ...
    $('table').on('click', 'tbody tr', function(){
        ...
        if(id)
        {
            window.location = BASE_URL+'/a_dMin/tbe-list/'+id;
        }
    });
    initPagination();
    $("body").on("click",".delete-ta",function () {
        var taId=$(this).data("id");
        $(".delete-conform-button").attr("data-id",taId);

        $("#deleteTaModal").modal("show");
    }) .on("click",'.delete-conform-button',function () {
        $(this).prop("disabled",true);
        var taId = $(this).attr("data-id");
        postData("/admin/admin/delete-ta", {ta_id:taId},function (response){
